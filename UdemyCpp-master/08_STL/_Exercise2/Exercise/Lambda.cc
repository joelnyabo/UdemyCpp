...
int main()
{
    ...
    const auto has_no_even =
        std::none_of(my_vector.begin(), my_vector.end(), [](const auto val){return (val%2) == 0;});
    ...
    // Exercise 2
    std::transform(my_vector.begin(), my_vector.end(), my_result.begin(), [](const auto val){ return val*4;});
